...
import { useAddAccountMutation, useAccountList } from "@/queries/useAccount";
...
export default function AddEmployee({ refetch }: AddEmployeeProps) {
  ...
  const onSubmit = async (values: CreateEmployeeAccountBodyType) => {
    ...
    try {
      ...
      toast.success(`${result.payload.data.name} đã được thêm vào hệ thống`);
      reset();
      refetch();
      setOpen(false);
    } catch (error) {
